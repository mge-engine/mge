...
def getGitRoot():
    RevParse = subprocess.Popen([Git, 'rev-parse', '--show-toplevel'],
                                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    return RevParse.stdout.read().strip().decode('utf-8')  # just a guess
...
def formatFile(FileName, GitRoot):
    subprocess.Popen(
        [ClangFormat, '-i', os.path.join(GitRoot, FileName)])
    return
...
def requiresFormat(FileName):
    ...
    ClangFormatRet = subprocess.Popen(
        [ClangFormat], stdin=GitShowRet.stdout, stdout=subprocess.PIPE)
