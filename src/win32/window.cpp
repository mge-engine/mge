...
namespace mge {
    namespace win32 {
        ...
        bool window::s_window_class_created;
        ...
        window::window(const mge::rectangle &     rect,
            ...
            : mge::window(rect, options), m_hwnd(0), m_update_listener(0)
        {
        ...
        window::~window()
        {
            ...
            if (m_msgthread) {
                if (mge::this_thread::get_id() == m_msgthread->get_id()) {
                    ...
                    // polling thread,
                    MGE_ERROR_LOG(WIN32)
                        << "Message thread cannot destroy own window!";
                    mge::crash();
                } else {
            ...
            if (w == nullptr) {
                ...
                case WM_CHAR:
                    if (wparam == 0x0D || wparam == 0x09 || wparam == 0x08) {
                        ...
                        w->add_message(
                            [w, wparam] { w->on_character((uint32_t)wparam); });
                    }
                    break;
                case WM_WANT_DESTROY:
                    MGE_DEBUG_LOG(WIN32)
                        << "Destruction of window " << hwnd << " requested";
                    w->m_hwnd = nullptr;
                    DestroyWindow(hwnd);
                    break;
                default:
        ...
        void window::add_message(std::function<void()> &&f)
        {
        ...
        void window::create_window()
        {
            ...
            RECT  window_rect = {rect().left(), rect().top(), rect().right(),
                                rect().bottom()};
            DWORD style;
            DWORD exstyle;
        ...
        window::msgthread::msgthread(window *w) : m_window(w) {}

        void window::msgthread::run()
        {
            ...
            MGE_DEBUG_LOG(WIN32)
                << "Leave message thread for window " << my_hwnd;
        }
        ...
        void window::on_show()
        {
            ...
            SetFocus(m_hwnd);
            m_update_listener =
                mge::application::instance().add_update_listener(
                    [&](uint64_t) { this->poll_events(); });
        }

        void window::on_hide()
        {
            mge::application::instance().remove_update_listener(
                m_update_listener);
            m_update_listener = 0;
