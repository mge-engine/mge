...
namespace mge {
    namespace win32 {
        ...
        monitor::monitor(const DISPLAY_DEVICEW &adapter_device,
            ...
            : mge::monitor(mge::to_utf8(device.DeviceString), primary),
              m_physical_width(0), m_physical_height(0)
        {
            ...
            if (!dc) {
                MGE_THROW(mge::system_error)
                    ...
                    << mge::to_utf8(adapter_device.DeviceName) << " failed";
            }
        ...
        unsigned int monitor::physical_height() const
        {
            return m_physical_height;
        }

        unsigned int monitor::physical_width() const
        {
            return m_physical_width;
        }

        monitor::monitor_collection_type monitor::all_monitors()
        {
            ...
            for (DWORD adapter_index = 0;; ++adapter_index) {
                ...
                if (!EnumDisplayDevicesW(nullptr, adapter_index,
                                         &adapter_device, 0)) {
                    break;
                }
                MGE_DEBUG_LOG(WIN32)
                ...
                for (DWORD display_index = 0;; ++display_index) {
                    ...
                    if (!EnumDisplayDevicesW(adapter_device.DeviceName,
                                             display_index, &display_device,
                                             0)) {
                        break;
                    }
                    result.emplace_back(std::make_shared<monitor>(
                        adapter_device, display_device, result.empty()));
                }
            }
            return result;
        }

        mge::monitor::video_mode_collection_type monitor::video_modes() const
        {
        ...
        void monitor::set_video_mode(const mge::video_mode &mode) { return; }

        mge::gamma_ramp monitor::gamma_ramp() const
        {
            WORD values[768];
            HDC dc = CreateDCW(L"DISPLAY", m_adapter_device.DeviceName, nullptr,
                               nullptr);
            if (!dc) {
                MGE_THROW(mge::system_error)
                    << "CreateDC call for device "
            ...
            memcpy(result.green_ramp(), values + 256,
                   256 * sizeof(unsigned short));
            memcpy(result.blue_ramp(), values + 512,
                   256 * sizeof(unsigned short));

            return result;
        }

        mge::video_mode monitor::current_video_mode() const
        {
            DEVMODEW dm;
            mge::zero_memory(dm);
            dm.dmSize = sizeof(DEVMODEW);
        ...
        mge::monitor_ref monitor::primary_monitor()
        {
