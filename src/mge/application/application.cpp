...
namespace mge {
    application::application()
        : m_argc(0), m_return_code(0),
          m_update_thread(std::make_shared<update_thread>()), m_update_rate(25),
          m_max_frame_skip(5), m_quit(false), m_setup_complete(false)
    {
        if (s_instance) {
    ...
    void application::add_application_thread(thread *t)
    {
        m_application_threads.add_thread(t);
    }

    void application::remove_application_thread(thread *t)
    {
    ...
    void application::join()
    {
        ...
        MGE_DEBUG_LOG(APPLICATION)
            << "Application shall quit, join all threads";
    ...
    void application::set_arguments(int argc_, const char **argv_)
    {
        if (m_argc) {
            MGE_THROW(mge::illegal_state)
    ...
    void application::parse_config_arguments()
    {
    ...
    void application::add_setup_task(const mge::task_ref &task)
    {
        if (m_setup_complete) {
            MGE_THROW(illegal_state)
                << "Setup tasks have already been processed";
        }
        if (!task) {
    ...
    void application::add_setup_task(std::function<void()> &&function)
    {
        auto tmp = std::make_shared<mge::task>(function);
        add_setup_task(tmp);
    }

    void application::await(const std::function<void()> &f)
    {
    ...
    void application::update(uint64_t ts)
    {
        std::lock_guard<decltype(m_lock)>                  guard(m_lock);
        function_map<void(std::uint64_t)>::iteration_scope iteration_guard(
            m_update_listeners);
        for (const auto &el : m_update_listeners) {
            try {
                el.second(ts);
            } catch (const mge::exception &e) {
                MGE_ERROR_LOG(APPLICATION)
                    << "Exception in update step: " << e.details();
            }
        }
    }
    ...
    bool application::remove_update_listener(
        function_map<application::update_listener>::key_type key)
    {
        return m_update_listeners.erase(key);
    }

    bool application::remove_quit_listener(
        function_map<application::quit_listener>::key_type key)
    {
    ...
    application::add_quit_listener(const application::quit_listener &listener)
    {
        if (listener) {
            ...
            MGE_THROW(illegal_argument) << "Try to add invalid quit listener";
        }
    }
    ...
    int application::run_application(int argc, const char **argv)
    {
        ...
        application::implementations(
            [&](const std::string &s) { implementations.emplace_back(s); });
