...
namespace mge {
    ...
    volatile update_thread *update_thread::s_update_thread;
    ...
    void update_thread::execute_work_item()
    {
    ...
    void update_thread::await(const std::function<void()> &f)
    {
    ...
    void update_thread::await(const task_ref &t)
    {
        m_tasks.push_back(t);
        t->wait();
    }
    ...
    void update_thread::run()
    {
        try {
            ...
            MGE_DEBUG_LOG(APPLICATION)
                << "Maximum skipped frames: " << max_frame_skip;
            while (!app.is_quit()) {
                uint32_t loops = 0;
