...
namespace mge {
     ...
     */
    class MGE_ASSET_EXPORT asset_type : boost::totally_ordered<asset_type>
    {
    public:
        ...
        asset_type(const std::string_view &type,
        ...
        asset_type(const asset_type &t)
            : m_type(t.m_type), m_subtype(t.m_subtype)
        {}

        asset_type(asset_type &&t)
            : m_type(std::move(t.m_type)), m_subtype(std::move(t.m_subtype))
        {}
        ...
        bool operator<(const asset_type &t) const
        {
            return m_type < t.m_type
                       ? true
                       : m_type == t.m_type ? m_subtype < t.m_subtype : false;
        }
    ...
    MGE_ASSET_EXPORT std::ostream &operator<<(std::ostream &    os,
                                              const asset_type &t);
