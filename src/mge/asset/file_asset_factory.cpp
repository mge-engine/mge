...
namespace mge {
    ...
    public:
        file_asset_access(const file &asset_file)
            : m_file(asset_file), m_properties_file(asset_file)
        {
            auto p = asset_file.path();
            if (!p.has_filename()) {
                MGE_THROW(illegal_argument)
                    << "Asset file name must have a file name component";
            }
        ...
        void configure(const configuration &config) override
        {
            ...
            if (!dir_file.is_directory()) {
                MGE_THROW(runtime_exception)
                    << "Directory '" << m_directory << "' does not exist";
            }
        }
    ...
    MGE_REGISTER_IMPLEMENTATION(file_asset_access_factory, asset_access_factory,
                                system, file);
} // namespace mge
