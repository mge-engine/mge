...
namespace mge {
    ...
    public:
        ...
        void add_mapping(const asset_type& t,
                         const char *suffixes)
        {
            ...
            for(auto& suffix : splitted) {
                ...
                if (it != m_suffix_map.end()) {
                    MGE_THROW(mge::illegal_state)
                            << "Asset type " << t << " already mapped to suffix " << it->first;
                }
    ...
    asset_type_mapper::asset_type_mapper(const asset_type &type, const char *suffixes)
    {
    ...
    asset_type
    asset_type_mapper::type(const mge::path &path)
    {
