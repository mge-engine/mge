...
namespace mge {
    ...
    private:
        bool update_loaders()
        {
            ...
            asset_loader::implementations([&](const std::string &name) {
                if (m_loader_implementations.find(name) ==
                    m_loader_implementations.end()) {
        ...
        void add_loader(const std::string &name)
        {
            ...
            loader->handled_types(
                [&](const asset_type &t) { handled_types.emplace_back(t); });
    ...
    asset_loader_ref asset_loader::loader(const asset &a)
    {
