...
namespace mge {
    ...
    scene::~scene() { scene_registry.destroy(m_scene_entity); }

    void scene::destroy_node(mge::entity node_entity)
    {
        m_registry.destroy(node_entity);
    }

    scene *scene::by_entity(mge::entity scene_entity)
    {
    ...
    mge::entity_registry &scene::registry() noexcept { return scene_registry; }
    ...
    bool scene::is_node_valid(const node &n) const
    {
        return m_scene_entity == n.m_scene_entity &&
               m_registry.valid(n.m_node_entity);
    }
