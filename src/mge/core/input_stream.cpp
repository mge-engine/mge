...
namespace mge {
    ...
    public:
        input_stream_buffer()
            : m_stream(0), m_buffer(0), m_buffer_size(0), m_buffer_used(0)
        {}

        input_stream_buffer(input_stream *stream)
            : m_stream(stream), m_buffer(0), m_buffer_size(0), m_buffer_used(0)
        {
            if (m_stream) {
                m_buffer      = new char[MOGE_STREAMBUF_BUFFER_SIZE];
                m_buffer_size = MOGE_STREAMBUF_BUFFER_SIZE;
                setg(m_buffer, m_buffer, m_buffer);
            }
        }

        input_stream_buffer(const input_stream_buffer &buffer)
            : std::streambuf(), m_stream(buffer.m_stream), m_buffer(0),
              m_buffer_size(0), m_buffer_used(0)
        {
            if (m_stream) {
    ...
    {
        decltype(m_read_listeners)::iteration_scope guard(m_read_listeners);
        for (const auto &e : m_read_listeners) {
            e.second(read_bytes);
        }
    }
} // namespace mge
