...
namespace mge {
    ...
    static singleton<std::map<std::string, shared_library_ref>>
                      loaded_libraries;
    ...
    {
        class shared_library_instantiate : public shared_library
        {
        public:
            inline shared_library_instantiate(const std::string &name,
                                              void *             handle)
...
#ifdef MGE_OS_WINDOWS
        std::lock_guard<decltype(loaded_libraries_mutex)> lock(
            loaded_libraries_mutex);
...
#    error Unsupported platform!
    ...
    void *shared_library::symbol(const char *name)
    {
#ifdef MGE_OS_WINDOWS
        ...
        if (!result || rc) {
            MGE_WARNING_LOG(MGE)
                << "Failed to look up symbol " << name << " in library "
                << m_name << ": " << rc << std::endl;
        } else {
            MGE_WARNING_LOG(MGE)
                << m_name << "(" << name << ") = " << result << std::endl;
        }
