...
namespace mge {
    ...
    private:
        bool is_module_file(const file &f) const
        {
            for (const auto &prefix : m_module_prefixes) {
                if (boost::starts_with(f.path().filename().generic_string(),
                                       prefix)) {
                    if (f.path().extension().generic_string() ==
                        shared_library::extension()) {
                        return true;
                    }
                }
            }
        ...
        void load_module(const file &module_file)
        {
            ...
            try {
                m_loaded_modules.emplace_back(
                    shared_library::load(module_file.path().string()));
            } catch (...) {
                MGE_WARNING_LOG(MODULE)
                    << "Load module " << module_file << " failed";
            }
    ...
    void module::load_all() { s_module_registry->load_all(); }
} // namespace mge
