...
namespace mge {
    ...
    template <typename T>
    void fill_stacktrace(HANDLE thread, CONTEXT *context, T &frames,
                         string_pool &strings)
    {
        ...
        while (frames.size() < 100) {
            BOOL rc =
                StackWalk64(IMAGE_FILE_MACHINE_AMD64, GetCurrentProcess(),
                            thread, &frame, context, NULL,
...
#    define MAX_SYMBOL_NAME_LEN 2000
            ...
            if (SymGetLineFromAddr64(current_process, frame.AddrPC.Offset,
                ...
                frames.emplace_back(
                    ...
                    strings.get(moduleinfo.ImageName), strings.get(methodname),
                    strings.get(lineinfo.FileName), lineinfo.LineNumber);
            } else {
                frames.emplace_back((void *)frame.AddrPC.Offset,
                                    strings.get(moduleinfo.ImageName),
                                    strings.get(""), strings.get(""), 0);
            }
        }
...
#endif
    stacktrace::frame::frame(const void *address, std::string_view module,
                             std::string_view name, std::string_view file,
                             uint32_t line)
        : m_address(address), m_name(name), m_source_file(file),
          m_module(module), m_source_line(line)
    {}
    ...
    stacktrace::stacktrace(const stacktrace &src)
    {
        for (const auto &f : src.m_frames) {
            m_frames.emplace_back(
                f.address(), m_strings.get(f.module()), m_strings.get(f.name()),
                m_strings.get(f.source_file()), f.source_line());
        }
    }
    ...
    stacktrace &stacktrace::operator=(const stacktrace &src)
    {
        if (&src != this) {
            ...
            for (const auto &f : src.m_frames) {
                tmp_frames.emplace_back(f.address(), m_strings.get(f.module()),
