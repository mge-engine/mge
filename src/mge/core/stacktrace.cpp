...
namespace mge {
    ...
    string_pool stacktrace::s_strings;
    std::mutex  stacktrace::s_strings_lock;

    stacktrace::stacktrace() { fill(); }

    stacktrace::stacktrace(const std::thread &t) { fill(t); }

    stacktrace::stacktrace(const thread &t) { fill(t.m_running_thread); }

    stacktrace::stacktrace(const stacktrace &s) : m_elements(s.m_elements) {}

    stacktrace::stacktrace(stacktrace &&s) : m_elements(std::move(s.m_elements))
    {}
    ...
    stacktrace::size_type stacktrace::size() const { return m_elements.size(); }
    ...
    std::ostream &operator<<(std::ostream &os, const stacktrace &b)
    {
        for (const auto &el : b) {
            if (el.file && *el.file) {
                os << (void *)el.address << " " << el.function << "("
                   << el.module << ") " << el.file << ":" << el.line
                   << std::endl;
            } else {
                os << (void *)el.address << " " << el.function << "("
                   << el.module << ")" << std::endl;
...
#    endif
    ...
    const char *stacktrace::intern_str(const char *str)
    {
    ...
    void stacktrace::fill(HANDLE thread_handle, CONTEXT *context)
    {
        ...
        }
        {
            ...
            while (size() < 100) {
                BOOL rc = StackWalk64(
                    IMAGE_FILE_MACHINE_AMD64, GetCurrentProcess(),
...
#    define MAX_SYMBOL_NAME_LEN 2000
                ...
                if (SymGetLineFromAddr64(current_process, frame.AddrPC.Offset,
                    ...
                    m_elements.emplace_back(element(
                        (void *)frame.AddrPC.Offset, intern_str(methodname),
                        intern_str(lineinfo.FileName), (int)lineinfo.LineNumber,
                        intern_str(moduleinfo.ImageName)));
                } else {
                    m_elements.emplace_back(element(
                        (void *)frame.AddrPC.Offset, intern_str(methodname),
                        intern_str(""), 0, intern_str(moduleinfo.ImageName)));
                }
#    ifdef MGE_COMPILER_GNUC
                m_elements.back().function =
                    demangle(m_elements.back().function.c_str());
#    endif
            }
    ...
    void backtrace::fill(const std::thread &) {}
#endif
