...
namespace mge {
    ...
    uint64_t clock::now()
    {
        ...
        if (current_epoch) {
            uint64_t current_now = static_cast<uint64_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count());
            return current_now - current_epoch;
        }

        s_epoch = static_cast<uint64_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count());
        return 0;
    }
