...
namespace mge {
    ...
    protected:
        void on_write(const void *buffer, streamsize_type size) override
        {
            ...
            if (written != (size_t)size) {
                MGE_THROW(io_error)
                    << "Cannot write to file '" << m_filename << "'";
            }
        }
    ...
    {
    public:
        system_file_input_stream(const std::string &filename) : m_file(nullptr)
        {
...
#    undef fseek
        ...
        input_stream::streamsize_type read() { return input_stream::read(); }

        input_stream::streamsize_type
        on_read(void *destination, input_stream::streamsize_type size) override
        {
            if (size < 0) {
                MGE_THROW(mge::illegal_argument) << "Read size must be >= 0";
            }

            streamsize_type readbytes =
                (streamsize_type)fread(destination, 1, (size_t)size, m_file);
            if (readbytes != size) {
                if (!feof(m_file)) {
                    MGE_THROW(mge::io_error)
                        << "File read failed, failed to read " << size
                        << " bytes";
                } else {
                    if (readbytes == 0) {
                        readbytes = -1;
    ...
    private:
        static int whence(direction_type dir)
        {
            ...
            default:
                MGE_THROW(mge::io_error)
                    << "Unsupported seek direction: " << dir;
            }
        }
        FILE *m_file;
    };
    ...
    {
    public:
        system_file_access(const mge::path &p) : file_access(p) {}
    ...
    bool system_file_access::is_system_file() const { return true; }
    ...
    MGE_REGISTER_IMPLEMENTATION(system_file_access_factory, file_access_factory,
                                system, default, file);

    file_access_ref
    system_file_access_factory::create_file_access(const mge::path &p)
    {
