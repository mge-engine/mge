...
namespace mge {
    ...
    properties::properties(const properties &p) : m_data(p.m_data) {}

    properties::properties(properties &&p) : m_data(std::move(p.m_data)) {}
    ...
    void properties::erase(const char *key) { m_data.erase(key); }
