...
namespace mge {
    ...
    archive::entry::entry(const mge::path &path, bool is_directory,
                          std::streamsize size, uint64_t index)
        : m_path(path), m_size(size), m_index(index), m_directory(is_directory)
    {}

    archive::entry::entry(const archive::entry &e)
        : m_path(e.m_path), m_size(e.m_size), m_access(e.m_access),
          m_index(e.m_index), m_directory(e.m_directory)
    {}

    archive::entry::entry(archive::entry &&e)
        : m_path(std::move(e.m_path)), m_size(e.m_size),
          m_access(std::move(e.m_access)), m_index(e.m_index),
    ...
    const mge::path &archive::entry::path() const { return m_path; }

    input_stream_ref archive::entry::open() { return m_access->open(m_index); }
    ...
    const archive::entry &archive::find_entry(const path &p) const
    {
        auto it = std::find_if(
            ...
            [&](const archive::entry &e) -> bool { return e.path() == p; });
        if (it == m_entries.end()) {
