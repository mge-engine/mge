...
TEST(input_handler, key_action_consumed)
{
    ...
    handler.set_key_action_handler(
...
TEST(input_handler, mouse_action_consumed)
{
    ...
    handler.set_mouse_action_handler([&](uint32_t             button,
                                         mge::mouse_action    action,
...
TEST(input_handler, mouse_move_consumed)
{
    ...
    handler.set_mouse_move_handler([&](uint32_t x, uint32_t y) {
        handler_called = true;
        EXPECT_EQ(x, 50u);
        EXPECT_EQ(y, 75u);
        return true; // consume
...
TEST(input_handler, mouse_move_not_consumed)
{
    ...
    handler.set_mouse_move_handler(
...
TEST(input_handler, mouse_wheel_consumed)
{
    ...
    handler.set_mouse_wheel_handler([&](int32_t x, int32_t y) {
        handler_called = true;
        EXPECT_EQ(x, 10);
        EXPECT_EQ(y, -5);
        return true; // consume
...
TEST(input_handler, mouse_wheel_not_consumed)
{
    ...
    handler.set_mouse_wheel_handler(
...
TEST(input_handler, character_consumed)
{
    ...
    handler.set_character_handler([&](uint32_t ch) {
        handler_called = true;
        EXPECT_EQ(ch, 'X');
        return true; // consume
    });
...
TEST(input_handler, character_not_consumed)
{
    ...
    handler.set_character_handler([&](uint32_t ch) { return false; });
