...
namespace mge {
    ...
    public:
        ...
        void on_mouse_move(int x, int y) { input_handler::on_mouse_move(x, y); }

        void on_character(unsigned int ch) { input_handler::on_character(ch); }

        void add_modifier(modifier_value m) { input_handler::add_modifier(m); }
    ...
    TEST(input_handler, on_key_action)
    {
        ...
        input_handler::key_action_handler h = [&](key k, key_action action,
                                                  modifier m) {
            EXPECT_TRUE(m.empty());
