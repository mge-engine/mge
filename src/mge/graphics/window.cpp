...
namespace mge {
    window::window(const rectangle &rect, const window_options &options)
        : m_rect(rect), m_options(options), m_visible(false), m_quit_listener(0)
    {}
    ...
    const render_context &window::render_context() const
    {
        return *m_render_context;
    }

    render_context &window::render_context() { return *m_render_context; }

    void window::assign_thread() { render_context().assign_thread(); }
    ...
    void window::install_display_thread()
    {
    ...
    void window::uninstall_display_thread()
    {
        if (m_display_thread) {
            MGE_DEBUG_LOG(WINDOW) << "Uninstall window display thread";
            m_display_thread->set_quit();
            application::instance().remove_application_thread(
                m_display_thread.get());
    ...
    void window::set_redraw_listener(const window::redraw_listener &listener)
    {
        m_redraw_listener = listener;
    }

    void window::clear_redraw_listener() { clear_function(m_redraw_listener); }
    ...
    void window::clear_close_listener() { clear_function(m_close_listener); }
    ...
    void window::refresh(mge::thread *display_thread, float interpolation)
    {
        if (m_redraw_listener) {
            try {
                redraw_context context{this, display_thread, render_context(),
                                       interpolation};
                m_redraw_listener(context);
            } catch (const mge::exception &ex) {
                MGE_DEBUG_LOG(WINDOW)
                    << "Exception in redraw: " << ex.details();
                m_redraw_listener = nullptr;
            }
        }
    }

    void window::close() { request_close(); }
    ...
    window::display_thread::display_thread(window *w)
        : m_window(w), m_quit(false)
    {}

    window::display_thread::~display_thread() {}

    void window::display_thread::set_quit() { m_quit = true; }

    void window::display_thread::run()
    {
        ...
        try {
            ...
            MGE_ERROR_LOG(WINDOW)
                << "Acquiring application object failed: " << ex;
            mge::rethrow();
        }
        ...
        while (!m_quit) {
            ...
            if (display_ts < next_game_tick) {
                ...
                interpolation =
                    (float)(next_game_tick + skip_ticks - display_ts) /
                    (float)skip_ticks;
            }
            try {
                ...
                MGE_ERROR_LOG(WINDOW)
                    << "Exception in window refresh: " << ex.what();
                m_quit = true;
            } catch (...) {
    ...
    void window::display_thread::exec_work_item()
    {
    ...
    void window::await(const std::function<void()> &f)
    {
