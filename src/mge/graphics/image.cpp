...
namespace mge {
    ...
    image::image(const mge::extent &e, const image_format &format)
        : m_extent(e), m_format(format), m_map_ptr(nullptr), m_map_counter(0)
    {}
    ...
    void *image::scanline(uint32_t line) const
    {
        ...
        return const_cast<uint8_t *>(ptr + line * extent().width() *
                                               pixel_size(format()));
    }
    ...
    {
        image_ref result =
            std::make_shared<memory_image>(extent(), format(), map());
        unmap();
        return result;
    }

    size_t image::buffer_size() const
    {
        return extent().area() * pixel_size(format());
    }

    rgba_color image::pixel_color(uint32_t x, uint32_t y) const
    {
        if (x > extent().width()) {
