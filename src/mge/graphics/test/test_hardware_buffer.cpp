...
TEST(hardware_buffer, map_unmap)
{
    ...
    mge::mock_hardware_buffer buffer(*context, mge::buffer_type::INDEX_BUFFER,
                                     mge::usage::DYNAMIC);
...
TEST(hardware_buffer, mappable_buffer)
{
    ...
    for (const auto &u : usage) {
        auto executor = std::make_shared<mge::mock_async_executor>();
        auto context =
            std::make_shared<mge::mock_render_context>(executor.get());
...
TEST(hardware_buffer, construct_unmapped_unmappable_throws)
{
    ...
    EXPECT_THROW_WITH_MESSAGE(
        mge::mock_hardware_buffer buffer(
            *context, mge::buffer_type::INDEX_BUFFER, mge::usage::IMMUTABLE),
        mge::illegal_argument, "Unsupported usage for unmapped buffer");
}
