...
namespace mge {
    TEST(hardware_buffer, map_unmap)
    {
        ...
        mge::mock_hardware_buffer buffer(
            *context, mge::buffer_type::INDEX_BUFFER, mge::usage::DYNAMIC);
    ...
    TEST(hardware_buffer, unmappable_buffer)
    {
        ...
        auto context =
            std::make_shared<mge::mock_render_context>(&system, executor.get());
        mge::mock_hardware_buffer unmappable_buffer(
            *context, mge::buffer_type::INDEX_BUFFER, mge::usage::IMMUTABLE,
    ...
    {
        mge::usage         usage[] = {mge::usage::DEFAULT, mge::usage::DYNAMIC,
                              mge::usage::STAGING};
        mock_render_system system;
        for (const auto &u : usage) {
            auto executor = std::make_shared<mge::mock_async_executor>();
            auto context  = std::make_shared<mge::mock_render_context>(
                &system, executor.get());
            mge::mock_hardware_buffer buffer(*context,
                                             mge::buffer_type::INDEX_BUFFER, u);
    ...
    TEST(hardware_buffer, construct_unmapped_unmappable_throws)
    {
        ...
        auto context =
            std::make_shared<mge::mock_render_context>(&system, executor.get());
        EXPECT_THROW_WITH_MESSAGE(mge::mock_hardware_buffer buffer(
                                      *context, mge::buffer_type::INDEX_BUFFER,
                                      mge::usage::IMMUTABLE),
