...
/**
 * @file rotating_triangle_slang.cpp
 * @brief Sample demonstrating Slang shader compilation with
 * compile_and_link.
 *
 * This sample is based on rotating_triangle but uses a single Slang
 * source file compiled via program::compile_and_link() instead of
 * separate per-API shader sources.
 */
...
namespace mge {
    MGE_DEFINE_TRACE(ROTATING_TRIANGLE_SLANG);
}
...
namespace mge {
    ...
    class rotating_triangle_slang : public application
    {
    public:
        rotating_triangle_slang() = default;

        void setup() override
        {
            MGE_DEBUG_TRACE(ROTATING_TRIANGLE_SLANG,
                            "Setup rotating_triangle_slang");
        ...
        void screenshot()
        {
            MGE_DEBUG_TRACE(ROTATING_TRIANGLE_SLANG, "Taking screenshot");
            ...
            if (m_initialized) {
                if (!m_uniform_block && !m_program->needs_link()) {
                    ...
                    if (!uniform_buffers.empty()) {
                        ...
                        MGE_ERROR_TRACE(ROTATING_TRIANGLE_SLANG,
        ...
        void initialize()
        {
            MGE_DEBUG_TRACE(ROTATING_TRIANGLE_SLANG, "Initializing objects");
            ...
            MGE_DEBUG_TRACE(ROTATING_TRIANGLE_SLANG, "Create vertex buffer");
            m_vertices = m_window->render_context().create_vertex_buffer(
                layout,
                sizeof(triangle_coords),
                mge::make_buffer(triangle_coords));

            MGE_DEBUG_TRACE(ROTATING_TRIANGLE_SLANG, "Create index buffer");
    ...
    MGE_REGISTER_IMPLEMENTATION(rotating_triangle_slang,
                                mge::application,
                                rotating_triangle_slang);
} // namespace mge
