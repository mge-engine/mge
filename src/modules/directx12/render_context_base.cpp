...
namespace mge::dx12 {
    ...
    render_context_base::render_context_base(mge::dx12::render_system& system,
                                             const mge::extent&        ext)
        : mge::render_context(system, ext)
        , m_render_system(system)
        , m_data_lock("render_context_base")
    {
        m_viewport    = {0.0f,
                         0.0f,
        ...
        m_scissor_rect = {0,
                           0,
                           static_cast<LONG>(ext.width),
                           static_cast<LONG>(ext.height)};
    ...
    void render_context_base::enable_debug_layer()
    {
    ...
    void render_context_base::create_factory()
    {
    ...
    void render_context_base::create_adapter()
    {
    ...
    void render_context_base::enable_debug_messages()
    {
        if (m_render_system.debug()) {
            ...
            if (SUCCEEDED(m_device.As(&m_info_queue))) {
                ...
                HRESULT rc =
                    m_info_queue->RegisterMessageCallback(&message_func,
    ...
    void render_context_base::message_func(D3D12_MESSAGE_CATEGORY category,
    ...
    void render_context_base::create_command_queue()
    {
    ...
    void render_context_base::create_descriptor_heap()
    {
        ...
        m_rtv_heap->SetName(L"mge::dx12::render_context_base::m_rtv_heap");
        ...
        CHECK_HRESULT(rc, ID3D12Device, CreateDescriptorHeap);
        m_dsv_heap->SetName(L"mge::dx12::render_context_base::m_dsv_heap");
        ...
        CHECK_HRESULT(rc, ID3D12Device, CreateDescriptorHeap);
        m_srv_heap->SetName(L"mge::dx12::render_context_base::m_srv_heap");
        m_srv_descriptor_size = m_device->GetDescriptorHandleIncrementSize(
            D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        m_srv_next_index = 0;
    }

    void render_context_base::create_command_lists()
    {
    ...
    void render_context_base::setup_backbuffer_as_render_target(
        ...
        DXGI_FORMAT& /*pass_dsv_format*/)
    {
        MGE_THROW(mge::illegal_state)
            << "DX12 context does not support rendering without a frame buffer";
    }

    void render_context_base::ensure_command_list_open()
    {
    ...
    render_context_base::create_command_allocator(D3D12_COMMAND_LIST_TYPE type,
                                                   const char*             purpose)
    {
        if (purpose) {
    ...
    render_context_base::create_dx12_command_list(
    ...
    void render_context_base::wait_for_command_queue()
    {
    ...
    void render_context_base::copy_resource_sync(ID3D12Resource*       dst,
                                                  ID3D12Resource*       src,
    ...
    void render_context_base::copy_texture_sync(
        ID3D12Resource*                           dst,
        ID3D12Resource*                           src,
        const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& footprint)
    {
        ...
        m_xfer_command_list->CopyTextureRegion(&dst_location,
                                               0,
                                               0,
                                               0,
                                               &src_location,
    ...
    render_context_base::allocate_srv()
    {
    ...
    render_context_base::rtv_handle(uint32_t index) const
    {
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE render_context_base::allocate_rtv()
    {
        D3D12_CPU_DESCRIPTOR_HANDLE result =
            m_rtv_heap->GetCPUDescriptorHandleForHeapStart();
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE
    render_context_base::dsv_handle(uint32_t index) const
    {
        D3D12_CPU_DESCRIPTOR_HANDLE result =
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE render_context_base::allocate_dsv()
    {
        D3D12_CPU_DESCRIPTOR_HANDLE result =
            m_dsv_heap->GetCPUDescriptorHandleForHeapStart();
    ...
    {
        auto fb =
            std::make_unique<dx12::frame_buffer>(*this);
    ...
    void render_context_base::render(const mge::pass& p)
    {
        ...
        if (!p.frame_buffer()) {
            setup_backbuffer_as_render_target(pass_command_list,
                                              pass_rtv,
                                              pass_dsv,
                                              pass_has_dsv,
                                              pass_rtv_format,
            ...
            auto* fb =
                static_cast<dx12::frame_buffer*>(p.frame_buffer().get());
    ...
    void render_context_base::draw_geometry(
        ...
        uint32_t                         index_offset)
    {
        auto dx12_prog = static_cast<dx12::program*>(program);
        if (!dx12_prog) {
            MGE_THROW(mge::illegal_state)
        ...
        const auto& pipeline_state =
            static_pipeline_state(dx12_prog, state, il, rtv_format, dsv_format);
        if (!pipeline_state.Get()) {
            MGE_THROW(mge::illegal_state)
                << "Failed to get pipeline state for program";
        }
        auto root_signature = dx12_prog->root_signature();
        command_list->SetGraphicsRootSignature(root_signature);
        command_list->SetPipelineState(pipeline_state.Get());

        if (ub) {
            bind_uniform_block(command_list, *dx12_prog, *ub);
        }
        ...
        if (!textures.empty()) {
            ...
            uint32_t base_texture_root_index =
                static_cast<uint32_t>(dx12_prog->uniform_blocks().size());
    ...
    render_context_base::input_layout_from_vertex_buffer(mge::vertex_buffer* vb)
    {
        return m_input_layout_cache.get(vb->layout());
    }

    const mge::com_ptr<ID3D12PipelineState>&
    render_context_base::static_pipeline_state(
        ...
        DXGI_FORMAT                                  dsv_format)
    {
        pipeline_state_key key = std::make_tuple(
            static_cast<void*>(program), state, rtv_format, dsv_format);
        ...
        auto rc = m_device->CreateGraphicsPipelineState(&pso_desc,
