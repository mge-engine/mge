...
namespace mge::dx12 {
    ...
    slang_compile_result slang_compile(std::string_view source)
    {
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(dx12::error) << "Failed to create Slang global session";
        }

        slang::TargetDesc target_desc = {};
        target_desc.format = SLANG_DXBC;
        target_desc.profile = global_session->findProfile("sm_5_0");
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(dx12::error) << "Failed to create Slang session";
        }
        ...
        if (!module) {
            ...
            MGE_THROW(dx12::error)
                << "Failed to load Slang module: " << diag_msg;
        }

        SlangInt32 entry_point_count = module->getDefinedEntryPointCount();
        if (entry_point_count == 0) {
            MGE_THROW(dx12::error) << "Slang module has no entry points";
        }
        ...
        for (SlangInt32 i = 0; i < entry_point_count; ++i) {
            ...
            if (SLANG_FAILED(rc)) {
                MGE_THROW(dx12::error) << "Failed to get entry point " << i;
            }
        ...
        if (SLANG_FAILED(rc)) {
            ...
            MGE_THROW(dx12::error)
                << "Failed to compose Slang program: " << diag_msg;
        }
        ...
        if (SLANG_FAILED(rc)) {
            ...
            MGE_THROW(dx12::error)
                << "Failed to link Slang program: " << diag_msg;
        }

        slang::ProgramLayout* layout =
            linked_program->getLayout(0, diagnostics.writeRef());
        if (!layout) {
            MGE_THROW(dx12::error) << "Failed to get Slang program layout";
        }
        ...
        for (SlangInt32 i = 0; i < entry_point_count; ++i) {
            ...
            if (SLANG_FAILED(rc)) {
                std::string diag_msg;
                if (diagnostics) {
                    diag_msg.assign(static_cast<const char*>(
            ...
            MGE_DEBUG_TRACE(DX12,
                            "Slang entry point {}: stage={}, "
                            "code size={} bytes",
                            i,
                            type,
                            code->getBufferSize());

            mge::buffer buf(code->getBufferSize());
            memcpy(buf.data(), code->getBufferPointer(), code->getBufferSize());

            result.shader_code[type] = std::move(buf);
        }
