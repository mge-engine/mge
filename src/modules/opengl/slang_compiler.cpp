...
namespace mge::opengl {
    ...
    slang_compile_result slang_compile(std::string_view source)
    {
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(opengl::error) << "Failed to create Slang global session";
        }

        slang::TargetDesc target_desc = {};
        target_desc.format = SLANG_GLSL;
        target_desc.profile = global_session->findProfile("glsl_410");
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(opengl::error) << "Failed to create Slang session";
        }
        ...
        if (entry_point_count == 0) {
            MGE_THROW(opengl::error) << "Slang module has no entry points";
        }
        ...
        for (SlangInt32 i = 0; i < entry_point_count; ++i) {
            ...
            if (SLANG_FAILED(rc)) {
                MGE_THROW(opengl::error) << "Failed to get entry point " << i;
            }
        ...
        if (!layout) {
            MGE_THROW(opengl::error) << "Failed to get Slang program layout";
        }
        ...
        for (SlangInt32 i = 0; i < entry_point_count; ++i) {
            ...
            MGE_DEBUG_TRACE(OPENGL,
                            ...
                            "GLSL source size={} bytes",
                            i,
                            type,
                            glsl_source.size());

            result.shader_code[type] = std::move(glsl_source);
        }
