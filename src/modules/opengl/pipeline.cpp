...
namespace opengl {
    pipeline::pipeline(mge::render_context &context)
        : mge::pipeline(context), m_program(0)
    {
        await([&] {
    ...
    void pipeline::clear_vaos()
    {
        if (m_program) {
            opengl::render_context &ctx =
                static_cast<opengl::render_context &>(context());
            ctx.clear_vaos_of_program(gl_program(*this));
        }
    }

    void pipeline::on_set_shader_program(const mge::shader_program_ref &shader_)
    {
        assert_same_context(*shader_);
        if (!shader_->defined()) {
            MGE_THROW(mge::illegal_state)
                << "Cannot apply undefined shader to pipeline";
        }

        const shader_program *opengl_shader =
            static_cast<const shader_program *>(shader_.get());
        await([&] {
    ...
    void pipeline::collect_attributes()
    {
        ...
        // MGE_DEBUG_LOG(OPENGL) << "Active pipeline attributes: " <<
        // active_attributes;
        ...
        // MGE_DEBUG_LOG(OPENGL) << "Active pipeline attribute max length: " <<
        // active_attributes_maxlength;
        ...
        for (GLuint i = 0; i < active_attributes; ++i) {
            ...
            GLsizei length = 0;
            GLint   size   = 0;
            glGetActiveAttrib(m_program, i, static_cast<GLsizei>(name.size()),
                              &length, &size, &type, name.data());
            CHECK_OPENGL_ERROR(glGetActiveAttrib);
            name.resize(static_cast<std::string::size_type>(length));
            m_attributes.emplace_back(mge::pipeline::attribute{
                name, data_type_of_gl_type(type), static_cast<uint32_t>(size)});
            // MGE_DEBUG_LOG(OPENGL) << "Attribute[" << i << "] = {name=\"" <<
            // name << "\", type=" << std::hex << type << ", size=" << size <<
            // "}";
        }
        ...
        // MGE_DEBUG_LOG(OPENGL) << "Active pipeline uniforms: " <<
        // active_uniforms;
        ...
        for (uint32_t i = 0; i < active_uniforms; ++i) {
            ...
            GLsizei length = 0;
            GLint   size   = 0;
            glGetActiveUniform(m_program, i, static_cast<GLsizei>(name.size()),
                               &length, &size, &type, name.data());
            CHECK_OPENGL_ERROR(glGetActiveUniform);
            name.resize(static_cast<std::string::size_type>(length));
            m_uniforms.emplace_back(mge::pipeline::uniform{
                name, data_type_of_gl_type(type), static_cast<uint32_t>(size)});
        }
    }
    ...
    void pipeline::dump_info_log()
    {
        ...
        if (loglength == 0) {
            ...
            glGetProgramInfoLog(m_program, length, &length,
