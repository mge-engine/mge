...
namespace opengl {
    ...
    texture_2d::texture_2d(mge::render_context & context,
        ...
        : mge::texture_2d(context, image), m_texture(0)
    {
        if (!image->mapped()) {
        ...
        await([&] {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, /* level */
                         gl_internal_format(image->format()), m_extent.width(),
                         m_extent.height(), 0, /* border */
                         gl_internal_format(image->format()), GL_UNSIGNED_BYTE,
                         image->data());
    ...
    texture_2d::texture_2d(mge::render_context &context)
        : mge::texture_2d(context), m_texture(0)
    {
        await([&] {
            glGenTextures(1, &m_texture);
            CHECK_OPENGL_ERROR(glGenTextures);
