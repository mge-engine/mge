...
namespace opengl {
    index_buffer::index_buffer(mge::render_context &context,
                               mge::data_type type, mge::usage usage,
                               size_t element_count, void *initial_data)
        : mge::index_buffer(context, type, usage, element_count, initial_data),
          m_buffer(0), m_size(element_count * mge::data_type_size(type))
    {
        await([&] {
            ...
            glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                         static_cast<GLsizeiptr>(size()), initial_data,
                         gl_usage(usage));
    ...
    size_t index_buffer::size() const { return m_size; }
