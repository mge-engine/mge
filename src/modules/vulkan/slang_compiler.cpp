...
namespace mge::vulkan {
    ...
    slang_compile_result slang_compile(std::string_view source)
    {
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(vulkan::error) << "Failed to create Slang global session";
        }
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(vulkan::error) << "Failed to create Slang session";
        }
        ...
        if (entry_point_count == 0) {
            MGE_THROW(vulkan::error) << "Slang module has no entry points";
        }
        ...
        for (SlangInt32 i = 0; i < entry_point_count; ++i) {
            ...
            if (SLANG_FAILED(rc)) {
                MGE_THROW(vulkan::error) << "Failed to get entry point " << i;
            }
        ...
        if (!layout) {
            MGE_THROW(vulkan::error) << "Failed to get Slang program layout";
        }
        ...
        for (SlangInt32 i = 0; i < entry_point_count; ++i) {
            ...
            shader_code.spirv_code.resize(code->getBufferSize());
            memcpy(shader_code.spirv_code.data(),
