...
namespace vulkan {
    ...
    render_system::render_system()
        : m_instance(VK_NULL_HANDLE), m_debug_messenger(VK_NULL_HANDLE),
          m_device(VK_NULL_HANDLE), m_graphics_queue(VK_NULL_HANDLE),
    ...
    void render_system::configure(const mge::configuration &config)
    {
        if (m_config.configured()) {
            MGE_THROW(mge::illegal_state)
                << "Render system is already configured";
        }
    ...
    mge::window_ref
    render_system::create_window(const mge::rectangle &     rect,
        ...
        for (const auto &name : m_instance_extensions) {
            ...
            if (!found) {
                MGE_THROW(vulkan::error)
                    << "Required extension " << name << " not found";
            }
        }
    }
...
#    pragma warning(pop)
    ...
    void render_system::scan_properties()
    {
        ...
        for (const auto &e : m_all_instance_extensions) {
            MGE_DEBUG_LOG(VULKAN)
                << "Found instance extension: " << e.extensionName;
        }
        ...
        for (const auto &l : m_all_instance_layers) {
            MGE_DEBUG_LOG(VULKAN) << "Found instance layer: " << l.layerName
                                  << "(" << l.description << ")";
        }
    }
        ...
        void *                                      user_data)
    {
        if (severity && VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT) {
            MGE_DEBUG_LOG(VULKAN) << "[" << callback_data->messageIdNumber
                                  << "][" << callback_data->pMessageIdName
                                  << "] : " << callback_data->pMessage;
        } else if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) {
            MGE_INFO_LOG(VULKAN) << "[" << callback_data->messageIdNumber
                                 << "][" << callback_data->pMessageIdName
                                 << "] : " << callback_data->pMessage;
        } else if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
            MGE_WARNING_LOG(VULKAN) << "[" << callback_data->messageIdNumber
                                    << "][" << callback_data->pMessageIdName
                                    << "] : " << callback_data->pMessage;
        } else {
            MGE_ERROR_LOG(VULKAN) << "[" << callback_data->messageIdNumber
                                  << "][" << callback_data->pMessageIdName
                                  << "] : " << callback_data->pMessage;
        }
    ...
    void render_system::init_debug_message_handling()
    {
        ...
        CHECK_VK_CALL(vkCreateDebugUtilsMessengerEXT(
    ...
    void render_system::load_physical_devices()
    {
        ...
        m_physical_device =
            m_physical_devices[m_selected_physical_device_index];
        ...
        vkGetPhysicalDeviceMemoryProperties(
            m_physical_device, &m_physical_device_memory_properties);
        ...
        fill_enumeration(
            [&](uint32_t *count, VkExtensionProperties *data) {
                CHECK_VK_CALL(vkEnumerateDeviceExtensionProperties(
                    m_physical_device, nullptr, count, data));
            },
            m_physical_device_extensions);

        MGE_DEBUG_LOG(VULKAN) << "Selected physical device: "
    ...
    uint32_t render_system::best_queue_family_index(VkQueueFlagBits flags)
    {
        if (flags & VK_QUEUE_COMPUTE_BIT) {
            for (uint32_t i = 0;
                 i < m_physical_device_queue_family_properties.size(); ++i) {
    ...
    {
        m_queue_family_indices.compute =
            best_queue_family_index(VK_QUEUE_COMPUTE_BIT);
        m_queue_family_indices.transfer =
            best_queue_family_index(VK_QUEUE_TRANSFER_BIT);
        m_queue_family_indices.graphics =
            best_queue_family_index(VK_QUEUE_GRAPHICS_BIT);

        MGE_DEBUG_LOG(VULKAN)
            << "Compute queue family index: " << m_queue_family_indices.compute;
        MGE_DEBUG_LOG(VULKAN) << "Transfer queue family index: "
                              << m_queue_family_indices.transfer;
        MGE_DEBUG_LOG(VULKAN) << "Graphics queue family index: "
                              << m_queue_family_indices.graphics;
    }
    ...
    void render_system::create_device()
    {
        ...
        compute_queue_create_info.sType =
            VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
        compute_queue_create_info.queueFamilyIndex =
            m_queue_family_indices.compute;
        ...
        create_info.ppEnabledExtensionNames =
            m_enabled_device_extensions.data();
        create_info.enabledExtensionCount =
            static_cast<uint32_t>(m_enabled_device_extensions.size());

        CHECK_VK_CALL(vkCreateDevice(m_physical_device, &create_info, nullptr,
                                     &m_device));
...
#define RESOLVE(X)                                                             \
    do {                                                                       \
        ...
        if (!f) {                                                              \
            MGE_WARNING_LOG(VULKAN)                                            \
                << "Cannot resolve device function: " << #X;                   \
        } else {                                                               \
            MGE_DEBUG_LOG(VULKAN) << "Resolve " << #X << ": " << (void *)f;    \
        }                                                                      \
        this->X = reinterpret_cast<decltype(this->X)>(f);                      \
...
#    pragma warning(pop)
    ...
    {
        vkGetDeviceQueue(m_device, m_queue_family_indices.graphics, 0,
                         &m_graphics_queue);
        vkGetDeviceQueue(m_device, m_queue_family_indices.compute, 0,
                         &m_compute_queue);
        vkGetDeviceQueue(m_device, m_queue_family_indices.transfer, 0,
                         &m_transfer_queue);
    }

    MGE_REGISTER_IMPLEMENTATION(render_system, mge::render_system, vulkan);
