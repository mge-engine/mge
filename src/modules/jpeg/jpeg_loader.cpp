...
namespace jpeg {
    ...
    public:
        ...
        static void error_exit(j_common_ptr cinfo)
        {
            ...
            jpeg_destroy(cinfo);
            //            throw AGGE_EXCEPTION(agge::asset::asset_corrupted)<<
            //            "JPEG image corrupted: "
            //            << buffer
            //            << ".";
        ...
        static mge::buffer read_fully(const mge::input_stream_ref &is)
        {
            ...
            is->read(result.data(), (mge::input_stream::streamsize_type)insize);
            return result;
        }
        ...
        std::any load(const mge::asset &a) const override
        {
            ...
            try {
                ...
                jpeg_mem_src(&cinfo, inbuffer.data(),
                             (unsigned long)inbuffer.size());
                ...
                auto img = std::make_shared<mge::memory_image>(
                    mge::extent(cinfo.output_width, cinfo.output_height), fmt);
                while (cinfo.output_scanline < cinfo.output_height) {
                    JSAMPROW row[1];
                    row[0] =
                        (unsigned char *)img->scanline(cinfo.output_scanline);
        ...
        void handled_types(
            std::function<void(const mge::asset_type &)> &&f) const override
        {
