...
namespace mge::dx11 {
    ...
    slang_compile_result slang_compile(std::string_view source)
    {
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(dx11::error) << "Failed to create Slang global session";
        }
        ...
        if (SLANG_FAILED(rc)) {
            MGE_THROW(dx11::error) << "Failed to create Slang session";
        }
        ...
        if (entry_point_count == 0) {
            MGE_THROW(dx11::error) << "Slang module has no entry points";
        }
        ...
        for (SlangInt32 i = 0; i < entry_point_count; ++i) {
            ...
            if (SLANG_FAILED(rc)) {
                MGE_THROW(dx11::error) << "Failed to get entry point " << i;
            }
        ...
        if (!layout) {
            MGE_THROW(dx11::error) << "Failed to get Slang program layout";
        }
