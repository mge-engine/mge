...
namespace dx11 {
    ...
    void shader_program::on_compile(const std::string &source_code)
    {
        ...
        auto rc = D3DCompile(source_code.c_str(), source_code.size(),
                             file_name.c_str(), nullptr, nullptr,
                             main_function.c_str(), shader_profile.c_str(), 0,
                             0, &code, &errors);
        if (FAILED(rc)) {
            ...
            errors->Release();
            throw dx11::error().set_info_from_hresult(rc, __FILE__, __LINE__,
                                                      "", "D3DCompile")
                << "(" << errormessage << ")";
        } else {
            m_code.reset(code);
    ...
    std::string shader_program::profile() const
    {
        switch (type()) {
