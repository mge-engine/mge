...
namespace dx11 {
    ...
    void command_list::clear(const mge::rgba_color &c)
    {
        assert_not_immutable();

        float clearcolor[4] = {c.r, c.g, c.b, c.a};
        m_deferred_context->ClearRenderTargetView(
            dx11_render_target_view(*m_context), clearcolor);
    }

    void command_list::draw(const mge::draw_command &cmd)
    {
        MGE_THROW_NOT_IMPLEMENTED;
    }

    void command_list::draw(mge::draw_command &&cmd)
    {
        MGE_THROW_NOT_IMPLEMENTED;
    }

    void command_list::clear_depth(float value)
    {
        MGE_THROW_NOT_IMPLEMENTED;
        // m_deferred_context->ClearDepthStencilView
    }

    void command_list::clear_stencil(int32_t value)
    {
        MGE_THROW_NOT_IMPLEMENTED;
        // m_deferred_context->ClearDepthStencilView
    }

    void command_list::on_finish()
    {
    ...
    void command_list::on_play(const mge::command_list_ref &commands)
    {
        MGE_THROW_NOT_IMPLEMENTED;
    }

    void command_list::on_clear()
    {
        ID3D11CommandList *command_list;
