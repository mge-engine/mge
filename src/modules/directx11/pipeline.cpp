...
namespace dx11 {
    pipeline::pipeline(mge::render_context &context)
        : mge::pipeline(context), m_vertex_shader_uniform_count(0)
    {}
    ...
    data_type_of_parameter(const D3D11_SIGNATURE_PARAMETER_DESC &desc)
    {
        if (desc.ComponentType == D3D_REGISTER_COMPONENT_FLOAT32 ||
            ...
            MGE_THROW(dx11::error)
                << "Unknown or unsupported attribute data type";
        }
    }
    ...
    void pipeline::reflect_vertex_shader(dx11::shader_program *s)
    {
        ID3D11ShaderReflection *shader_reflection = nullptr;
        D3DReflect(s->code()->GetBufferPointer(), s->code()->GetBufferSize(),
                   IID_ID3D11ShaderReflection, (void **)&shader_reflection);
        if (shader_reflection) {
            ...
            for (uint32_t i = 0; i < shader_desc.ConstantBuffers; ++i) {
                ...
                if (strcmp(cbuffer_desc.Name, "$Globals") == 0) {
                    ...
                    MGE_THROW(dx11::error)
                        << "Constant buffers not yet supported";
                }
            }
    ...
    void pipeline::on_link()
    {
        ...
        dx11::shader_program *vs = static_cast<dx11::shader_program *>(
            m_shaders[(size_t)mge::shader_type::VERTEX].get());
        reflect_vertex_shader(vs);
        m_vertex_shader_uniform_count = m_uniforms.size();
        // dx11::shader *ps = static_cast<dx11::shader
        // *>(m_shaders[(size_t)mge::shader_type::FRAGMENT].get());
        // reflect_shader(ps);
    }

    void pipeline::on_set_shader_program(const mge::shader_program_ref &s)
    {
